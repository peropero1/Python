...
def UpdateSk(element,Sk_head,Sk):
    ...
    if Sk_head[row].count>e_max.count:
        Update_emax(Sk_head,Sk,element)
...
# ==========================update e_max==========================
def Update_emax(head,sketch,item):
    ...
    '''
    e_max=get_emax()
    local_max_col,local_max_row=position(item)
    if head[local_max_row].maxID=='':
        return 0
    else:
        if sketch[local_max_row][local_max_col]>e_max.count:
            e_max.ID=head[local_max_row].maxID
            e_max.count=sketch[local_max_row][local_max_col] 
