...
def UpdateSk(element,Sk_head,Sk):
    ...
    for i in range(Config.depth):
        col=position(element,i)
        Sk_head[i].distinct.add(element.ID)
        Sk_head[i].count+=element.count
        Sk[i][col]+=element.count
        if Sk_head[i].maxID=='':
            Sk_head[i].maxID=element.ID
            Sk_head[i].keep=element.count
            lhh_index=col
        elif Sk_head[i].maxID==element.ID:
            Sk_head[i].keep+=element.count
            lhh_index=col
        else:
            # head_item.maxID != element.ID:
            local_max_col=position(DS.Tail(Sk_head[i].maxID,0),i)
            if local_max_col==col:
                ...
                if Sk_head[i].keep<0:
                    ...
                    Sk_head[i].keep=element.count
                    lhh_index=col
            elif Sk[i][col]>Sk[i][local_max_col]:
                Sk_head[i].keep-=element.count
                if Sk_head[i].keep<0:
                    Sk_head[i].maxID=element.ID
                    Sk_head[i].keep=element.count
    ...
    if int((Sk[lhh_index]+head_item.keep)/2)>Config.e_max.count:    
