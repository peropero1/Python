...
def single_query(e,Top_dict,Sk_head,Sketch,cardinality):
    # e is element.ID
    count=0
    if Top_dict.get(e):
        count=Top_dict[e]
        ...
        col,row=position(DS.Tail(e,0))
        row_cardinality=cardinality[row]
            # distinct element in Sketch[row]
        ratio=Config.width/row_cardinality
            # avg collision rate
        # e in Sketch
        avg_count=Sk_head[row].count/row_cardinality
            # avg count per cell 
        if e == Sk_head[row].maxID:
            # e is LHH
            if ratio<=1:
